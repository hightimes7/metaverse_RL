...
public class Main : MonoBehaviour
{
	...
	float xAngle = 0; 
	float yAngle = 0;		// 카메라가 보고 있는 각도
	
	// Track view mode camera parameters
	...
	void Update()
	{
        ...
        {
			// 1. 현재 프레임 시간만큼 남은 시간을 없앤다.
			toastMessageRemain -= Time.deltaTime;
			// 2. 남은 시간이 0이하이면, 시간이 모두 흐른 것이므로 toastMessage를 사라지게 함
	...
	void FixedUpdate()
	{
		...
		if(ss[0] == "join")
		{
			if(!spawns.ContainsKey(ss[1]))
			{
				...
				// 새로운 플레이어가 접속했다고 메시지를 토스트에 뿌립니다.
		...
		else if(ss[0] == "action")
		{
			...
			if(worldItems.ContainsKey(ss[2]))
			{
				...
				// 3. 만약 world item에 join하는 명령어인경우 해당 캐릭터를 앉도록 함
				if(ss[3] == "join")
                {
					var spawn = spawns[ss[1]];
					spawn.GetAvatar().Sit();
				...
				else if (ss[3] == "leave")
                {
					...
					spawn.joinItem = null;
					// 토스트 메세지 표시
					var mesg = string.Format("{0}가 {1}에서 떠났습니다.", ss[1], ss[2]);
					ShowToastMessage(mesg);
				}				
				else if(ss[3] == "quit")
                {
	...
	void UpdateFirstView()		
    {
		...
		Camera.main.transform.localEulerAngles = 
			new Vector3(dir.x + xAngle, dir.y + yAngle, dir.z);
    }

	void UpdateTrackView()
    {
		// 만약 c키가 눌리면 카메라 모드를 일인칭 뷰로 바꿈
		if (Input.GetKeyUp(KeyCode.C))
		{
	...
	public void ShowMessageBox(string mesg, string yes)
    {
		...
		// 3. Message 오브젝트에 mesg 값을 적기
		messageObj.text = mesg;
		// 4. yes 버튼이 눌릴 경우 전달할 메세지를 저장
		yesSendMesg = yes;
